...
// Random high port per run — a stale server from a prior run must never be hit.
...
server.kill('SIGTERM');
// Give the OS time to release the port and confirm the process exited.
try { await new Promise((resolve) => server.once('exit', resolve)); } catch { /* already exited */ }
