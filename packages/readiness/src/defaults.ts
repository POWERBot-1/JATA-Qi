...
export const DEFAULT_CAPABILITIES: Capability[] = [
  ...
  cap({ id: 'policy-governance', name: 'Policy & governance control plane (versioned, tenant-aware, precedence, agent/tool governance)', category: 'governance', status: 'PARTIALLY_IMPLEMENTED', module: '@jataqi/policy-governance', evidence: ['rich versioned policy model + multi-effect engine', 'precedence (deny>safety>legal>org>role>entitlement; sensitive default-deny)', 'org/role/entitlement integration, simulation, overrides, autonomy levels, agent governance — 14 tests'], notes: 'engine + registry real & tested; not yet wired as a MANDATORY enforcement gate on every action across all runtime modules (tool-intelligence/orchestrator still enforce their own gates); UI absent' }),
