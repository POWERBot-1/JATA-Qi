...
/**
 * T-09: currency is now a REQUIRED argument for every scale-dependent money
 * helper (no silent 2-decimal default). This T-07 regression suite passes the
 * 2-decimal currency 'KES' explicitly, so every assertion below keeps exactly
 * the value it had before T-09 — the 2dp behaviour is unchanged (AC-3).
 *
...
describe('T-07 money policy (commercial-control-plane money utilities)', () => {
  ...
  it('derives exact minor units without float multiplication', () => {
    ...
    assert.equal(minorUnitsOf(123456789.99, 'KES'), 12345678999n);
    // 1.005 is NOT 100.5 minor units in float math; the decimal parse must win.
    assert.equal(minorUnitsOf(1.005, 'KES'), 101n); // 100.5 -> half-up -> 101
  });

  it('quantizes at the boundary: <=2dp amounts pass through bit-identical', () => {
    assert.ok(isQuantizedAmount(0.1, 'KES'));
    assert.ok(isQuantizedAmount(19.99, 'KES'));
    assert.ok(!isQuantizedAmount(1.005, 'KES'));
    assert.ok(!isQuantizedAmount(0.1 * 3, 'KES')); // float product 0.30000000000000004 carries >2 dp
    assert.ok(!isQuantizedAmount(Number('59.969999999999995'), 'KES')); // parsed decimal carries >2 dp
    assert.equal(quantizeAmount(19.99, 'KES'), 19.99);
    assert.equal(quantizeAmount(0.3, 'KES'), 0.3);
  });

  it('rounds half-up on the exact decimal value (never on float arithmetic)', () => {
    assert.equal(quantizeAmount(1.005, 'KES'), 1.01); // float 1.005*100 < 100.5 would round down
    assert.equal(quantizeAmount(2.675, 'KES'), 2.68); // classic float trap
    assert.equal(quantizeAmount(1.004, 'KES'), 1.0);
    assert.equal(quantizeAmount(0.1 + 0.2, 'KES'), 0.3); // float sum 0.30000000000000004 rounds to exactly 0.3
    assert.equal(quantizeAmount(0.1 * 3, 'KES'), 0.3); // float product 0.30000000000000004 rounds to exactly 0.3
    assert.equal(quantizeAmount(Number('59.969999999999995'), 'KES'), 59.97); // 19.99 x 3-style artifact value rounds to exactly 59.97
    assert.equal(quantizeAmount(1e-7, 'KES'), 0); // below the minor unit -> zero (documented)
  ...
  it('sums deterministically: 0.1 + 0.2 === 0.3 exactly', () => {
    ...
    assert.equal(sumMonetaryAmounts([0.1, 0.2, 0.3], 'KES'), 0.6);
    assert.equal(sumMonetaryAmounts([19.99, 19.99, 19.99, 0.01], 'KES'), 59.98);
